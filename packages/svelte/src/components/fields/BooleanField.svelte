<script lang="ts">
  ...
  $: if (schema && currentValue == null) {
    void form.patch(path, defaultValue<boolean>(schema, currentValue ?? null) ?? false);
  }
</script>
...
<Wrap {schema} component={components.wrapper} errors={getErrorTreeAtPath($errorStore, path)}>
  <input
    ...
    on:change={(event) => void form.patch(path, (event.currentTarget as HTMLInputElement).checked)}
